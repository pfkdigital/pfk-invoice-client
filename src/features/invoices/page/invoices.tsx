    ...
    useEffect(() => {
        if (!isPlaceholderData && hasMore) {
            queryClient.prefetchQuery({
                ...
                queryFn: () => getInvoices(page + 1, limit, search, sort).then((data) => {
                    if (data.length === 0) {
                        setHasMore(false);
                    }
                    return data;
    ...
    return (
        <>
            ...
            <div className="flex items-center justify-end px-6 pb-8">
                <Pagination>
                    <PaginationContent>
                        <PaginationItem onClick={() => page === 1 ? setPage(1) : setPage(page - 1)}>
                            <PaginationPrevious />
                        </PaginationItem>
                        <PaginationItem>
                        ...
                        <PaginationItem onClick={() => hasMore ? setPage(page + 1) : setPage(page)}>
                            <PaginationNext />
                        </PaginationItem>
                    </PaginationContent>
                </Pagination>
